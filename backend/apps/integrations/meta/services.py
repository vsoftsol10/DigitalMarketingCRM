...
from .crypto import decrypt_token, encrypt_token
from .exceptions import MetaAPIError
from .oauth import OAUTH_ACTION_CONNECT, OAUTH_ACTION_RECONNECT
from .models import (
    MetaAccountCredential,
    MetaCredentialStatus,
    MetaCredentialType,
...
class MetaOAuthService:
    ...
    # ========================================================
    # RECONNECT EXISTING FACEBOOK ACCOUNT
    # ========================================================
    ...
    def reconnect_existing_account(
        ...
        *,
        ...
        """
        Reconnect one existing Facebook SocialAccount.

        Does not create a new SocialAccount.
        """
        ...
        if not social_account:
            raise MetaAPIError(
                "The Facebook account selected for reconnect " "was not found.",
            )
        ...
        if not target_page_id:
            raise MetaAPIError(
                "The Facebook Page ID is missing.",
            )

        # ----------------------------------------------------
        # VERIFY EXACT PAGE WITH META
        # ----------------------------------------------------
        ...
        if not isinstance(page, dict):
            raise MetaAPIError(
                "Meta returned an invalid Facebook Page response.",
            )
        ...
        if returned_page_id != target_page_id:
            raise MetaAPIError(
                "The Facebook Page could not be verified.",
            )

        # ----------------------------------------------------
        # FRESH PAGE ACCESS TOKEN
        # ----------------------------------------------------
        ...
        if not page_access_token:
            raise MetaAPIError(
                "Meta did not return a Facebook Page access token.",
            )
        ...
        # ----------------------------------------------------
        # GET / RESTORE META CONNECTION
        # ----------------------------------------------------

        connection = social_service._get_or_create_connection(
            provider_user_id=provider_user_id,
        )

        # ----------------------------------------------------
        # UPDATE META USER CREDENTIAL
        # ----------------------------------------------------

        social_service._save_user_credential(
            connection=connection,
            user_access_token=user_access_token,
            token_expires_at=token_expires_at,
        )
        ...
        social_account.account_name = (
            ...
            or social_account.account_name
            or target_page_id
        )
        ...
        # ----------------------------------------------------
        # REPLACE PAGE CREDENTIAL
        # ----------------------------------------------------

        social_service._save_page_credential(
            social_account=social_account,
    ...
    def prepare_oauth(
        ...
        *,
        ...
        organization,
        user,
        action: str = OAUTH_ACTION_CONNECT,
        ...
        CONNECT:
            ...
            Multiple Pages → Selection session
        ...
        RECONNECT:
            ...
            Discover Facebook Pages
                ↓
            Resolve existing SocialAccount
                ↓
            Verify exact Facebook Page
                ↓
        ...
        # ----------------------------------------------------
        # OAUTH ACTION VALIDATION
        # ----------------------------------------------------

        action = (
            str(
                action or OAUTH_ACTION_CONNECT,
            )
            .strip()
            .lower()
        )
        ...
        if action == OAUTH_ACTION_RECONNECT:
            return self.reconnect_existing_account(
                organization=organization,
                user=user,
